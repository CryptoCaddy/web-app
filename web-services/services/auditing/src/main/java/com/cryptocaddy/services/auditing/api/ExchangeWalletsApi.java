...
public interface ExchangeWalletsApi {
    ...
    @ApiOperation(value = "Exchange Wallets", notes = "The Exchange Wallets API gets wallet balances for all wallets from all saved exchanges", response = ResponseUserData.class, tags = {"Exchange Wallets"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Add Exchanges Response", response = ResponseUserData.class),
    ...
    ResponseEntity<ResponseUserData> getExchangeWallets(@RequestHeader(value="Authorization") String authorization);
